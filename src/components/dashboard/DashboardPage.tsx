...
const subjects: [string, number][] = [["Polity", 72], ["History", 54], ["Economics", 41]];
    ...
    <div className="app-shell">
      <aside className="app-sidebar">
        ...
        <nav className="sidebar-nav">
          {navigation.map(({ label, icon: Icon, href }) => <a key={label} href={href} className={`sidebar-link ${label === "Dashboard" ? "active" : ""}`}><Icon size={17} strokeWidth={1.9} /><span>{label}</span></a>)}
        </nav>
      ...
      <main className="app-main">
        ...
        <section className="dashboard-grid">
          <article className="panel plan-panel"><div className="panel-heading"><div><p className="eyebrow">TODAY</p><h2>Study plan</h2></div><a href="#">View plan</a></div><div className="plan-list"><PlanItem time="09:00" subject="Polity" topic="Parliament" status="done" /><PlanItem time="11:00" subject="Economics" topic="Inflation & monetary policy" status="current" /><PlanItem time="15:00" subject="Polity" topic="Revision — Fundamental Rights" status="upcoming" /><PlanItem time="18:00" subject="Mixed GS" topic="50 practice questions" status="upcoming" /></div></article>
          <article className="panel progress-panel"><div className="panel-heading"><div><p className="eyebrow">SYLLABUS</p><h2>Subject progress</h2></div><a href="#">Open syllabus</a></div><div className="subject-list">{subjects.map(([name, value]) => <div className="subject-row" key={name}><div className="subject-meta"><span>{name}</span><strong>{value}%</strong></div><div className="subject-track"><div className="subject-fill" style={{ width: `${value}%` }} /></div></div>)}</div><div className="progress-note"><span>12 topics completed this week</span><strong>+8%</strong></div></article>
        </section>
        <section className="bottom-grid">
          <article className="panel health-panel"><div className="panel-heading"><div><p className="eyebrow">PREPARATION HEALTH</p><h2>You&apos;re moving in the right direction.</h2></div></div><div className="health-content"><div className="health-score">78<span>/100</span></div><div className="health-bars"><HealthRow label="Syllabus" value="On track" kind="good" /><HealthRow label="Study time" value="Slightly behind" kind="warn" /><HealthRow label="Revision" value="On track" kind="good" /><HealthRow label="Tests" value="On track" kind="good" /></div></div></article>
